            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Test argocd authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Test argocd failed to authenticate properly"
                throw err
            }
        }
    }
        ...
        try {
            dir(projectDirectory) {
                ...
                slackSend color: "good",
                        message: "Test argocd deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test argocd failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test argocd shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test argocd failed to shut down properly"
            throw err
        }
    }
}
